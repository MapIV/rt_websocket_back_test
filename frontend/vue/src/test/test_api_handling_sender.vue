...
const url = "api.robot-controller.map4.jp";
...
function initWebsocket(topic: string,path: string) {
  ...
  if (path === "webcamera") {
    ws_map.value[topic] = new WebSocket(`wss://${url}/ws/webcamera/sender`); // test
  } else {
    ws_map.value[topic] = new WebSocket(`wss://${url}/ws`);
    console.log("`wss://${url}:8888/ws`");
  }
